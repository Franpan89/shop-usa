...
import { invitePortalClient } from '../../../actions/portal';
...
export default function EditClientForm({ client, categories }: EditClientFormProps) {
  ...
  async function handleInvite() {
    ...
    try {
      await invitePortalClient(client.id);
      setInviteMessage({ text: '✅ Invitación enviada', ok: true });
    } catch (err: any) {
      setInviteMessage({ text: err.message || 'Error al invitar', ok: false });
    } finally {
    ...
    <div className="glass-panel" style={{ padding: '32px' }}>
      ...
      <form onSubmit={handleSubmit} className="edit-client-form">
        ...
        <div className="form-grid">
          ...
          <div className="input-container">
            <label>Correo (para portal y notificaciones)</label>
            <input name="email" type="email" className="input-field" defaultValue={client.email || ''} />
          </div>
          <div className="input-container">
        ...
        <div className="form-actions" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            {client.email && (
              ...
              >
                {invitePending ? '⏳ Invitando...' : '📧 Invitar al Portal'}
              </button>
            )}
            {inviteMessage && (
              <span style={{ fontSize: '0.85rem', color: inviteMessage.ok ? '#10b981' : '#ef4444', fontWeight: 600 }}>
                {inviteMessage.text}
              </span>
            )}
          </div>
